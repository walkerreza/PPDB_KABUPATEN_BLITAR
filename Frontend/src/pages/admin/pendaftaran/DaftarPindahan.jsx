...
import PindahanForm from '../components/PindahanForm';
...
const RegistrationForm = React.memo(() => {
  ...
  const [formData, setFormData] = useState({
    ...
    jalur_pendaftaran: '3',    // ID jalur pendaftaran (3 = Pindahan)
  ...
  useEffect(() => {
    if (userData?.sekolah?.id_sekolah) {
      setFormData(prev => ({
        ...
        jalur_pendaftaran: '3' // Set default jalur pendaftaran ke Pindahan
      }));
    }
  }, [userData]);
  ...
  useEffect(() => {
    const fetchJalurPendaftaran = async () => {
      try {
        ...
        // Set default jalur pendaftaran ke Pindahan
        const pindahan = options.find(opt => opt.label.toLowerCase() === 'pindahan');
        if (pindahan) {
          setFormData(prev => ({
            ...prev,
            jalur_pendaftaran: pindahan.value
          }));
        }
      } catch (error) {
  ...
  // Handler untuk menangani pengiriman form pendaftaran
    ...
    try {
      ...
      if (response.ok) {
        ...
        setFormData({
          ...
          nama_sekolah_manual: '',
          tahun_lulus: '',
          jalur_pendaftaran: '3', // Set kembali ke jalur pindahan
          sekolah_tujuan: userData?.sekolah?.id_sekolah || '',
          keterangan: ''
        });
...
// State untuk data form warga Blitar
const [formData, setFormData] = useState({
  ...
  // Fungsi untuk mengambil data pendaftar pindahan
  const fetchData = async () => {
    try {
      ...
      const response = await fetch(
        `${import.meta.env.VITE_API_URL}/pendaftaran/data-pendaftar-pindahan/${userData.sekolah.id_sekolah}`,
        {
          headers: {
            'Authorization': `Bearer ${userData.token}`
          }
  ...
  const isSMP = useCallback((value) => {
    if (!value) return false;
    return ['311', '312'].includes(String(value));
  }, []);
      ...
      try {
        ...
        if (isSMP(userSchoolType)) {
          const response = await axios.get(
            `${import.meta.env.VITE_API_URL}/jadwal-pendaftaran/jadwal-sistem/18`
          );
          const { tanggal_mulai, tanggal_selesai } = response.data.data.jadwal;
          setJadwalData(prev => ({
            ...prev,
          ...
          const startTime = moment(tanggal_mulai);
          const endTime = moment(tanggal_selesai);
          const isOutsideTimeRange = !currentTime.isBetween(startTime, endTime, null, '[]');
          setIsPendaftaranDitutup(isOutsideTimeRange);
        }
      } catch (error) {
        console.error('Error saat mengecek jadwal:', error);
        setIsPendaftaranDitutup(true);
      }
    };

    checkJadwalStatus();
  }, [userData?.sekolah?.id_tipe_sekolah, isSMP]);
  ...
  useEffect(() => {
    const checkJadwalWargaBlitar = async () => {
      try {
        ...
        if (!response.ok) {
          throw new Error('Gagal mengecek jadwal');
        }
  ...
  return (
    <AdminGuard>
                ...
                <div className="mb-4">
                  <div className="flex border-b border-gray-200">
                    ...
                    >
                      ...
                      Daftar Siswa Sudah Mendaftar Pindahan
                    </button>
                  </div>
                </div>
                    ...
                    {isPendaftaranDitutup ? (
                      ...
                      <div className="bg-red-100 rounded-lg p-8 text-center">      
                        ...
                        <p className="text-red-600">
                          Pendaftaran online Program Penerimaan Siswa Baru Kabupaten Blitar untuk jalur Pindahan sudah ditutup.
                        </p>
                      </div>
                    ) : kuotaPenuh ? (
                      ...
                      <div className="bg-yellow-100 p-4 rounded-md border border-yellow-200 mt-4">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="font-medium">Maaf, Maksimum Pagu Pindahan Sudah Terpenuhi!</span>
                        </div>
                      </div>
                    ) : (
                      <div className="mt-4">
      ...
      <DeleteDialog
        ...
        description="Apakah Anda yakin ingin menghapus data pendaftar ini? Data yang sudah dihapus tidak dapat dikembalikan."
      />
