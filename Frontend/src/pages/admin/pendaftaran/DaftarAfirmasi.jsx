...
import AfirmasiForm from '../components/AfirmasiForm';
...
const RegistrationForm = React.memo(() => {
  ...
  const [formData, setFormData] = useState({
    ...
    jalur_pendaftaran: '4',    // ID jalur pendaftaran (4 = Afirmasi)
  ...
  // State untuk warga Kabupaten Blitar
  ...
  useEffect(() => {
    if (userData?.sekolah?.id_sekolah) {
      setFormData(prev => ({
        ...
        jalur_pendaftaran: '4' // Set default jalur pendaftaran ke Afirmasi
      }));
    }
  }, [userData]);
  ...
  useEffect(() => {
    const fetchJalurPendaftaran = async () => {
      try {
        ...
        // Set default jalur pendaftaran ke Afirmasi
        const afirmasi = options.find(opt => opt.label.toLowerCase() === 'afirmasi');
        if (afirmasi) {
          setFormData(prev => ({
            ...prev,
            jalur_pendaftaran: afirmasi.value
          }));
        }
      } catch (error) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setFormData({
          ...
          nama_sekolah_manual: '',
          tahun_lulus: '',
          jalur_pendaftaran: '4', // Set kembali ke jalur afirmasi
          sekolah_tujuan: userData?.sekolah?.id_sekolah || '',
          keterangan: ''
        });
  ...
  // Effect untuk mengecek status jadwal checkbox warga Blitar
  useEffect(() => {
  ...
  return (
    <form onSubmit={handleSubmit} className="max-w-4xl mx-auto space-y-6">
        ...
        <div className="bg-blue-50 p-4 rounded-lg">
          <label className="flex items-center space-x-2">
            <input
              ...
              onChange={(e) => setIsWargaBlitar(e.target.checked)}
      ...
      {/* Data Diri Section */}
      <div>
        <hd3 className="text-xl font-semibold mb-4 flex items-center">
            ...
            <div className="col-span-2 mb-4">
              ...
              <div className="flex items-center gap-4">
                <label className="inline-flex items-center">
                  ...
                  />
                  <span className="ml-2">
                    {jalurPendaftaranOptions.find(opt => opt.value === formData.jalur_pendaftaran)?.label || 'Afirmasi'}
                  </span>
                </label>
              </div>
            </div>
...
const DaftarAfirmasi = () => {
  ...
  // Tambahkan state untuk cek pendaftaran ditutup
  ...
  const fetchData = async () => {
    try {
      ...
      const response = await fetch(
        `${import.meta.env.VITE_API_URL}/pendaftaran/data-pendaftar-afirmasi/${userData.sekolah.id_sekolah}`,
        {
          headers: {
            'Authorization': `Bearer ${userData.token}`
          }
  ...
  useEffect(() => {
    const checkKuota = async () => {
      try {
        ...
        const url = `${import.meta.env.VITE_API_URL}/pendaftaran/kuota/jalur/4/${storedUserData.id_sekolah}`;
  ...
  useEffect(() => {
    const checkJadwalPendaftaran = async () => {
      try {
        ...
        if (!userSchoolType) {
          throw new Error('Tipe sekolah tidak ditemukan');
        }

        // Tentukan ID jadwal berdasarkan tipe sekolah
        let jadwalId;
        if (isSDMI(userSchoolType)) {
          jadwalId = 3; // ID untuk jalur Afirmasi SD
          console.log('Mengecek jadwal pendaftaran afirmasi SD dengan ID:', jadwalId);
        } else if (isSMP(userSchoolType)) {
          jadwalId = 12; // ID untuk jalur Afirmasi SMP
          console.log('Mengecek jadwal pendaftaran afirmasi SMP dengan ID:', jadwalId);
        } else {
          throw new Error('Tipe sekolah tidak valid');
        }

        // Panggil API untuk mengecek jadwal pendaftaran
        const url = `${import.meta.env.VITE_API_URL}/jadwal-pendaftaran/${jadwalId}`;
        console.log('Mengecek jadwal pendaftaran afirmasi:', url);
        ...
        if (!response.ok) {
          const errorText = await response.text();
          console.error('Response error:', response.status, errorText);
          throw new Error(`Gagal mengecek jadwal: ${errorText}`);
        }

        const data = await response.json();
        console.log('Data jadwal pendaftaran afirmasi:', data);

        if (data.status && data.data) {
          const jadwal = data.data;
          
          // Simpan jadwal ke state berdasarkan tipe sekolah
          if (isSDMI(userSchoolType)) {
            setJadwalData(prev => ({
              ...prev,
              sd: { 
          ...
          // Cek apakah saat ini masih dalam periode pendaftaran
          const currentTime = moment();
          const startTime = moment(jadwal.tanggal_mulai);
          const endTime = moment(jadwal.tanggal_selesai);
          
          // Jika saat ini di luar periode pendaftaran atau status jadwal tidak aktif
          ...
          // Set status pendaftaran ditutup jika di luar range waktu atau status tidak aktif
          setIsPendaftaranDitutup(isOutsideTimeRange || isJadwalInactive);
          
          console.log('Status pendaftaran afirmasi:', {
            tipeSekolah: isSDMI(userSchoolType) ? 'SD/MI' : 'SMP/MTs',
            currentTime: currentTime.format('YYYY-MM-DD HH:mm:ss'),
            startTime: startTime.format('YYYY-MM-DD HH:mm:ss'),
            endTime: endTime.format('YYYY-MM-DD HH:mm:ss'),
            isOutsideTimeRange,
            isJadwalInactive,
            isDitutup: isOutsideTimeRange || isJadwalInactive
          });
        } else {
          console.warn('Data jadwal tidak valid:', data);
          // Default ke pendaftaran ditutup jika data tidak valid
          setIsPendaftaranDitutup(true);
        }
      } catch (error) {
        console.error('Error saat mengecek jadwal pendaftaran:', error);
        // Default ke pendaftaran ditutup jika terjadi error
        setIsPendaftaranDitutup(true);
      }
    };

    // Jalankan fungsi pengecekan jadwal
              ...
              <Card className="mt-6">
                <div className="p-2 sm:p-4">
                  ...
                  <div className="mb-4">
                    <div className="flex border-b border-gray-200">
                      ...
                      >
                        ...
                        Daftar Siswa Sudah Mendaftar Afirmasi
                      </button>
                    </div>
                  </div>
                  ...
                  {activeTab === 'pendaftaran' && (
                    <>
                      ...
                      {isPendaftaranDitutup ? (
                        // Tampilkan pesan jika pendaftaran ditutup
                        <div className="bg-red-100 rounded-lg p-8 text-center">      
                          <h2 className="text-2xl font-bold text-red-800 mb-4">
                            Pendaftaran sudah ditutup!
                          </h2>
                          <p className="text-red-600">
                            Pendaftaran online Program Penerimaan Siswa Baru Kabupaten Blitar untuk jalur Afirmasi sudah ditutup.
                          </p>
                        </div>
                      ) : kuotaPenuh ? (
                        ...
                        <div className="bg-yellow-100 p-4 rounded-md border border-yellow-200 mt-4">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="font-medium">Maaf, Maksimum Pagu Afirmasi Sudah Terpenuhi!</span>
                          </div>
                        </div>
                      ) : (
                        <div className="mt-4">
                  ...
                  {activeTab === 'daftar' && (
                    <div className="mt-4">
                      <TableContainer
                        title="Data Pendaftar Afirmasi"
                        subtitle="Daftar siswa yang mendaftar melalui jalur Afirmasi"
                      >
                        {loading ? (
